...
export async function insertInitialUser() {
  ...
  try {
    parseCSV(
      "/home/yoyo/personal/dev/sifa/backend/tugas-akhir/data/seed tugas akhir farmasi - User.csv",
      (results) => {
        results.forEach(async (result) => {
          const username = result.username;
          const name = result.name;
          const groupAccess = Number(result.grupakses);

          await UserService.insertNewUserBySuperUser({
            username,
            name,
            groupAccess,
          });
